...
def make_cover(student_name, principal_name, office, period, num_entries, total_hours):
    ...
    # Declaration
    decl_rows = [
        [Paragraph("Principal's Declaration",
            S('dh', fontSize=11, fontName='Helvetica-Bold', textColor=BLUE)), '', ''],
    ...
    decl.setStyle(TableStyle([
        ...
        ('LINEBELOW',  (0,7),(-1,7), 0.5, BORDER),
        ('TOPPADDING', (0,0),(-1,-1),2),('BOTTOMPADDING',(0,0),(-1,-1),2),
        ('LEFTPADDING',(0,0),(-1,-1),14),('RIGHTPADDING',(0,0),(-1,-1),14),
        ('TOPPADDING', (0,0),(-1,0), 12),('BOTTOMPADDING',(0,8),(-1,8),8),
        ('VALIGN',     (0,0),(-1,-1),'MIDDLE'),
    ]))
...
def make_progress_report(report, period_label):
    ...
    # Header
    head = Table([[
        Paragraph("PRINCIPAL'S PROGRESS REPORT",
            S('prht', fontSize=9, fontName='Helvetica-Bold', textColor=WHITE, letterSpacing=1)),
        Paragraph(period_label,
            S('prhs', fontSize=10, fontName='Helvetica-Bold', textColor=WHITE, alignment=TA_RIGHT))
    ...
    items.append(Paragraph("To be completed every six (6) months.",
        S('note', fontSize=8, fontName='Helvetica', textColor=MUTED, spaceAfter=5)))

    # Principal observations
    items.append(Paragraph("Observations on training to date, experience gained and ability of Trainee:",
        S('prl', fontSize=8, fontName='Helvetica-Bold', textColor=MUTED, spaceAfter=3)))
    obs_box = Table([[Paragraph(obs if obs else '[Pending]',
        S('prb', fontSize=10, fontName='Helvetica', textColor=TEXT, leading=15))]], colWidths=[166*mm])
    ...
    items.append(Table([[
        ...
        Paragraph("_______________________________", S('sv1', fontSize=9, fontName='Helvetica', textColor=TEXT)),
        Paragraph("Date:", S('sl2', fontSize=9, fontName='Helvetica-Bold', textColor=MUTED)),
        Paragraph(fmt_date(psig) if psig else '________________', S('sd1', fontSize=9, fontName='Helvetica', textColor=TEXT)),
    ]], colWidths=[22*mm,70*mm,16*mm,58*mm], style=TableStyle([
        ('TOPPADDING',(0,0),(-1,-1),6),('BOTTOMPADDING',(0,0),(-1,-1),6),('LEFTPADDING',(0,0),(-1,-1),0),
    ])))
    items.append(Spacer(1,4*mm))

    # Candidate comments
    items.append(Paragraph("Candidate's Comments:",
        S('prl2', fontSize=8, fontName='Helvetica-Bold', textColor=MUTED, spaceAfter=3)))
    cand_box = Table([[Paragraph(cand if cand else '[To be completed by candidate]',
        S('prb2', fontSize=10, fontName='Helvetica', textColor=TEXT, leading=15))]], colWidths=[166*mm])
    ...
    items.append(Table([[
        ...
        Paragraph("_______________________________", S('csv1', fontSize=9, fontName='Helvetica', textColor=TEXT)),
        Paragraph("Date:", S('csl2', fontSize=9, fontName='Helvetica-Bold', textColor=MUTED)),
        Paragraph(fmt_date(csig) if csig else '________________', S('csd1', fontSize=9, fontName='Helvetica', textColor=TEXT)),
    ]], colWidths=[22*mm,70*mm,16*mm,58*mm], style=TableStyle([
        ('TOPPADDING',(0,0),(-1,-1),6),('BOTTOMPADDING',(0,0),(-1,-1),6),('LEFTPADDING',(0,0),(-1,-1),0),
    ])))
...
def make_entry(entry, num, total):
    ...
    items.append(Paragraph(f"{parish}  ·  {fmt_date(date)}  ·  {title_ref}  ·  Parcel: {parcel}",
        S('em', fontSize=9, fontName='Helvetica', textColor=MUTED, spaceAfter=5)))
    items.append(HRFlowable(width="100%", thickness=0.5, color=BORDER))
    ...
    for row in hour_rows:
        if row.get('hours'):
            right.append(Paragraph(f"{row.get('type','')}: <b>{row.get('hours','')} hrs</b>",
                S('whr', fontSize=9, fontName='Helvetica', textColor=TEXT, spaceAfter=2, leading=13)))
    right.append(Paragraph(f"<b>Total: {total_hrs} hrs</b>",
        S('wht', fontSize=9, fontName='Helvetica-Bold', textColor=BLUE, spaceAfter=3)))
...
def generate_pdf():
    try:
        ...
        filter_period    = data.get('filter_period', None)  # e.g. "January – June 2025"

        # Filter entries if a period is selected
        if filter_period:
        ...
        story += make_cover(f"{student_name}, BSc.(Hons.)", principal_name, office,
                            filter_period if filter_period else period,
                            len(entries), th_fmt)
        story += make_tally(entries)

        # Group entries by 6-month period — progress report BEFORE each group
        current_period = None
        entry_num = 0
        ...
        for e in entries:
            ...
            if lbl and lbl != current_period:
                # New period — insert progress report header first
